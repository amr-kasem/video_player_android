...
import io.flutter.plugins.videoplayer.Messages.TrackGroupMessage;
import io.flutter.plugins.videoplayer.Messages.SubtitleMessage;
import io.flutter.plugins.videoplayer.Messages.TrackFormatMessage;
import io.flutter.plugins.videoplayer.Messages.TextureMessage;
import io.flutter.plugins.videoplayer.Messages.VolumeMessage;
import io.flutter.plugins.videoplayer.Messages.TrackSelectionMessage;
...
public class VideoPlayerPlugin implements FlutterPlugin, AndroidVideoPlayerApi {
  ...
  public void setTrackGroup(@NonNull TrackSelectionMessage arg) {
    VideoPlayer player = videoPlayers.get(arg.getTextureId());
    player.setTrackGroup(arg.getGroupIndex(),arg.getFormatIndex());
  }

  public @NonNull TracksMessage getTracks(@NonNull TextureMessage arg) {
    VideoPlayer player = videoPlayers.get(arg.getTextureId());
    Tracks tracks = player.getTracks();
    ImmutableList<Group> groups = tracks.getGroups();
    return new TracksMessage.Builder()
                .setTextureId(arg.getTextureId())
                .setGroups(
                  IntStream.range(0, groups.size())
                  .mapToObj(
                    g_i -> {
                      Group g = groups.get(g_i);
                      ArrayList<TrackFormatMessage> formats = new ArrayList<TrackFormatMessage>();
                      ArrayList<Boolean> trackSelected = new ArrayList<Boolean>();
                      ArrayList<Boolean> trackSupported = new ArrayList<Boolean>();
                      for (int i = 0; i < g.length; i++) {
                        // Individual track information.
                        trackSupported.add(g.isTrackSupported(i));
                        trackSelected.add(g.isTrackSelected(i));
                        Format format = g.getTrackFormat(i);
                        formats.add(
                          new TrackFormatMessage.Builder()
                                      .setTextureId(arg.getTextureId())
                                      .setId(format.id)
                                      .setLabel(format.label)
                                      .setLanguage(format.language)
                                      .setFormatIndex(Long.valueOf(i))
                                    .build()
                        ) ;
                      }
                      return new TrackGroupMessage.Builder()
                              .setTextureId(arg.getTextureId())
                              .setAdaptiveSupported(g.isAdaptiveSupported())
                              .setTrackSelected(trackSelected)
                              .setTrackSupported(trackSupported)
                              .setFormats(formats)
                              .setTrackType(Long.valueOf(g.getType()))
                              .setGroupIndex(Long.valueOf(g_i))
                            .build();
                    }
                  )
  ...
  public @NonNull SubtitleMessage getCurrentSubtitle(@NonNull TextureMessage arg) {
    VideoPlayer player = videoPlayers.get(arg.getTextureId());
    SubtitleMessage result =
        new SubtitleMessage.Builder()
            .setValue(player.getCurrentSubtitleCue())
            .setTextureId(arg.getTextureId())
            .build();
    return result;
  }
